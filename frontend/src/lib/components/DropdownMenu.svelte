...
{#if visible}
  ...
  >
    {#each items as item}
      <button type="button" class="glassy-light button-option" on:click={() => { item.onClick ? item.onClick() : onSelect(item); }}>
        {item.label}
      </button>
    {/each}
  </div>
...
<style>
  .dropdown-menu {
    ...
    box-shadow: 0 8px 32px var(--glassy-shadow), 0 2px 8px var(--glassy-shadow-light);
