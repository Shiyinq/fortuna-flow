...
async def create_api_key(
    ...
    try:
        ...
        logger.info(f"[CREATE_API_KEY] Success: user_id={current_user.userId}")
        return new_api_key
...
async def delete_api_key(
    current_user=Depends(get_current_user), _=Depends(require_csrf_protection)
):
    """
    ...
    try:
        ...
        logger.info(f"[DELETE_API_KEY] Success: user_id={current_user.userId}")
        return deleted
