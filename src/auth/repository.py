...
async def find_refresh_token(query: dict):
    return await database["refresh_tokens"].find_one(query)
...
async def delete_refresh_token(token: str):
    return await database["refresh_tokens"].delete_one({"refreshToken": token})
